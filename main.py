...
from dics import members, bitly_yt_dic, bitly_yt_dic
...
all_num = lem(member_all)

#数値を取得して，条件を満たせばツイートする．
def number_notification(members_, num):
    start_          = time.time()
    num_log_file    = num_log_file.format(num)
    tweet_head      = '【達成通知】\n\n'
    ...
    #th_val人刻みでツイートするための条件設定．th_val = 1000 なら1000人刻み
...
#botが過去10ツイートと同じツイートしようとしたらキャンセルする(はず)
recent_tweets = api.user_timeline(count = 10)
recent_tweets = [tweet.text for tweet in recent_tweets]
def _tw_channel(tweet):
...
with open(data_log_file, 'wb') as pi:
    pickle.dump(contents, pi)
